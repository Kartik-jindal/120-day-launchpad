...
3. PARTITION BY <column(s)>: This is the "magic." It divides the dataset into independent groups or "windows."
The function is applied and calculated separately for each partition. When the value of the partitioning column
changes, the function resets.

4. ORDER BY <column(s)>: This sorts the rows *within each partition*. This is essential for functions that depend on order,
like RANK(), LEAD(), and LAG().
...
Aggregate Functions:
SUM(column): "Calculates the cumulative sum of the specified column within the partition."
AVG(column): "Calculates the average of the specified column within the partition."
COUNT(column): "Counts the number of non-null values in the specified column within the partition."     
Offset Functions:
